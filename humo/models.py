...
class Molecule(models.Model):
    ...
    def calculate_fingerprint(self):
        ...
        return [val for val in fingerprint]
    ...
    def calculate_coulomb(self):
        ...
        eigenvalues = eigenvalues[0].real
        if len(eigenvalues):
            return [float(v) for v in np.sort(eigenvalues)[::-1]]

    def calculate_adjacency(self):
        matrix = Chem.rdmolops.GetAdjacencyMatrix(self.molecule)
        eigenvalues, _ = np.linalg.eig(matrix)
        return [float(v) for v in np.sort(eigenvalues.real)[::-1]]
    ...
    def load_molecules(cls, csv_file, num_cores, start_index, end_index, testing=True):
        ...
        print (len(smile_gap_tuples), "THE LEN")
